...
# -*- coding: utf-8 -*-
"""
ClickUp Agent - Sistema de Criação de Demandas
Versão: 2.1.0
Autor: Manus AI
Data: 2025-06-30
...
class ClickUpAPI:
    ...
    def create_task(self, list_id: str, task_data: Dict) -> Tuple[bool, str]:
        ...
        success, response = self._make_request('POST', f"list/{list_id}/task", task_data)
...
def processar_demanda(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Preparar dados da tarefa principal
        task_data = {
            'name': tarefa,
            'description': f"""
**Tipo:** {tipo.title()}
...
{data.get('descricao', '')}
            ...
            'priority': data.get('prioridade', 3),
            'tags': data.get('tags', []),
            'custom_fields': [
                {
                    'id': 'empresa',
...
def home():
    ...
    return jsonify({
        'message': 'ClickUp Agent API v2.1.0',
        'status': 'online',
        'endpoints': {
...
def test_system():
    ...
    test_data = {
        'empresa': 'Teste Sistema',
        'tarefa': 'Validar funcionamento do sistema',
...
if __name__ == '__main__':
    ...
    logger.info(f"Iniciando ClickUp Agent v2.1.0 na porta {port}")
